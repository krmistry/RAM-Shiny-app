# **** Original version of app, with only regions and the colors are FisheryType rather
#      than taxGroup
...
data$Region <- data$Region %>% 
  ...
  gsub("European Union", "European Union (non Mediterranean)", .) 
# region names as they appear on website graphics
...
# Creating dataframe for min, max and scale type for MSY, Catch and Productivity for 
# each region:
...
for (i in 1:length(region_plot_titles)){
  ...
  dimnames(size_scale_per_region[[i]]) <- list(c("MSY", "Catch", "Productivity"), 
                                               c("min", "max"))
