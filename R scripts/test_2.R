...
ui <- fluidPage(
  ...
  pageWithSidebar(
    ...
    sidebarPanel(
      ...
      wellPanel(
        yearSliderUI("Year_slider", 
                     all_data = data1,
                     segment_data = split[[1]]),
        
        sizeDropdownUI("size_dropdown"),
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    # updateSliderInput(session, "Year_slider", "Year",
    #                   min = min(data1$Year),
    #                   max = max(data1$Year),
    #                   value = max(min(region_data[, 2]), min(data1$Year)))
    callModule(session, yearSlider, "Year_slider", all_data = data1,
               segment_data = region_data)
  ...
  # Reactive variables:
  Year_slider <- callModule(yearSlider, "Year_slider")
  size_values <- callModule(sizeDropdown, "size_dropdown",
                            size_scale_list = size_scale_per_region,
                            dataframe_name = myRegion)
  ...
  output$plot <- renderPlot({
    ggplot(myData(), aes(x = BoverBMSY,
                         y = UoverUMSY,
                         color = taxGroup)) +
      geom_point(aes(size = !!as.symbol(size_values$mySize())),
                 alpha = 0.8) +
      ...
      scale_size_continuous(name = size_values$size_legend_title(),
                            limits = c(size_values$min_size(), 
                                       size_values$max_size()),
                            labels = scales::comma) +
      ylim(0, 3) +
      xlim(0, 3) +
