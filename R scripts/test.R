...
server <- function(input, output, session) {
  ...
  observe({
    ...
    # If individual taxGroups and stocks are selected:
    if (mytaxGroup != "All" & myStock == "All") {
      region_data <- filter(region_data,
                            taxGroup %in% mytaxGroup)
    } else if (mytaxGroup != "All" & myStock != "All") {
      region_data <- filter(region_data,
                            taxGroup %in% mytaxGroup,
                            ID == myStock)
    } else if (mytaxGroup == "All" & myStock != "All") {
    ...
    updateSliderInput(session, "Year","Year", 
                      min = min(data1$Year), 
                      max = max(data1$Year), 
                      value = max(min(region_data$Year), min(data1$Year)))
  })
